...
- Sealing uses the GCP confidential-VM vTPM (`/dev/tpmrm0`). A wrong boot/VM context fails the `PolicyPCR` unseal — a sealed blob cannot be replayed on another VM (see `operator-guide.md` (Measured-boot chain and sealing)).
