...
| Path | Purpose |
|---|---|
| `etc/systemd/system/story-kernel-measure-binary.service` | PCR 12 extend (GCP CVM vTPM) |
| `etc/systemd/system/story-kernel-rtmr3-extend.service` | RTMR3 self-extend |
| `etc/systemd/system/story-kernel.service` | The DKG kernel itself (config from instance metadata) |
| `etc/systemd/network/20-wired.network` | GVNIC DHCP networking |
...
The services form the boot ordering chain:

```
measure-binary (PCR 12)  ──►  rtmr3-extend (RTMR3)  ──►  story-kernel
```
