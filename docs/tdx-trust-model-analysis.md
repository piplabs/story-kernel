...
## 부록 C: Option D 4-validator devnet 검증 (2026-05-12)

Option D decomposed whitelist의 실제 거동을 검증하기 위해 4-validator devnet을 두 시나리오로 운영했습니다.

### 구성
- **Validators (4, min_req=4)**:
  - `story-sgx` (Azure DC4s_v3, SGX) — `0x00a842dbD3D11176B4868DD753a552B8919d5A63`
  - `story-tdx` (Azure DC4es_v5, TDX paravisor) — `0x87f3Cc50c84005f7130D37b849f6a71E05a8Bf1F`
  - `story-gcp` (GCP c3-standard-4, TDX direct) — `0x8d0C35c2483BB207C0be04F239C300B37322384F`
  - `story-gcp-2` (GCP c3-standard-4, TDX direct — 같은 type pair) — `0xf8f30Edf53235069DE379A50124E369736F1F878`
- **Chain**: `story-68931` (local profile, engine chain id 90930)
- **Attestation**: `MockAutomataDcap` 로 DCAP cert chain 우회 (Option D 검증 목적 — cert chain은 직교 sub-system)

### 측정된 raw 48-byte (SHA-384) 값
| 항목 | Azure paravisor | GCP direct |
|---|---|---|
| MRTD | `ff450b138c5ee6734a43dc35afb437abfa24ef743bc9e9484ef4af365fcd89a6c50913248ed7fcb92a7d42b1ba7da984` | `feb7486608382c1ff0e15b4648ddc0acea6ca974eb53e3529f4c4bd5ffbaa20bf335cb75965cea65fe473aed9647c162` |
| RTMR0 | `00...00` (paravisor에서 0) | `70e9cd9b25c0277e61f9aa328a6346fb684de307babd31c17cd9aeecf9f8f75a6e8ccc5822a09a4b6d3db529c0adfb6c` |
| RTMR1 | `00...00` | `c041916ac1f5592fff0ce4cdf1c94b96870ae5786d857f605179f73ce6e9114892f29f8463c8ff2d27af6174f98acba4` |
| RTMR2 | `00...00` | `261eb562e22a8468350019ef9a979dca3e6d1b9f8ab6db7a9544c19086e5e1b4b437e80696d34f8cd4da42999dbcab34` |
| cloudKey = keccak(MRTD‖RTMR0) | `0xafc8bf33...3cafc7a4` | `0xe79e9e35...88886bf` |
| binaryKey = keccak(RTMR1‖RTMR2) | `0x46700b4d...0c08c21` | `0xf5c0db03...22e64d87` |

### Test 1 — 모든 type whitelist → 4 validators 모두 통과 ✅

**Whitelist (setTDXValidationHook)**:
- `approveCloudPlatform(TDX_AZURE_MRTD, TDX_AZURE_RTMR0, "azure-paravisor")`
- `approveBinaryRelease(TDX_AZURE_RTMR1, TDX_AZURE_RTMR2, "azure-paravisor-bootstrap")`
- `approveCloudPlatform(TDX_GCP_MRTD, TDX_GCP_RTMR0, "gcp-direct")`
- `approveBinaryRelease(TDX_GCP_RTMR1, TDX_GCP_RTMR2, "gcp-direct-bootstrap")`
- SGX `whitelistEnclaveType(1, EnclaveTypeData{codeCommitment: MRENCLAVE, ...})`

**On-chain Finalized 이벤트**:

| Round | Block | Validators finalized |
|------:|------:|---------------------|
| 4 (initial DKG) | 2101–2102 | **4/4** ✅ |
| 5 (resharing) | 2801–2802 | **4/4** ✅ |

4개 enclave 모두 Option D 검증 통과 → DKG keygen + resharing 두 차례 정상 완료. 동일 type (TDX-GCP) validator 2개가 같은 `cloudKey/binaryKey` 로 양쪽 모두 통과 — cloud SKU horizontal scaling 정상 동작 확인.

### Test 2 — Azure approval만 제외 → 1 validator 거부, DKG setup 실패 ✅

**Whitelist 변경**: `setTDXValidationHook` 에서 Azure 의 `approveCloudPlatform/Release` 호출만 의도적으로 생략 (GCP TDX 와 SGX 만 승인).

**On-chain Register 결과 (round 6, fresh chain)**:

| Validator | Type | 결과 |
|-----------|------|------|
| `0x00a842...` (story-sgx) | SGX | ✅ Registered at block 1047 |
| `0x8d0c35...` (story-gcp) | TDX-GCP | ✅ Registered at block 1142 |
| `0xf8f30e...` (story-gcp-2) | TDX-GCP | ✅ Registered at block 1141 |
| `0x87f3cc...` (story-tdx) | TDX-Azure | ❌ `TDXValidationHook: unapproved cloud platform` |

Story-tdx 의 register 호출이 hook에서 정확한 revert 메시지로 거부됨:
```
failed to estimate gas: execution reverted: TDXValidationHook: unapproved cloud platform
```

Registration period 종료 후 (block 1201) CL이 round skip 결정:
```
Verified registration count below minimum required.
Skipping current round. verified_count=3 min_req_registered=4 current=6 next=7
```

→ Min_req=4 미충족으로 DKG round 6 dealing 진입 못 함 → DKG setup 실패. **Option D whitelist gating 이 cloud-platform 수준의 fine-grained 거부 + 격리된 vendor toggle 을 보장함을 실데이터로 입증**.

### 디버깅 픽스 (검증 과정에서 발견된 두 건)

1. **`TDXValidationHook: Attestation failed` 차단**: 새 V3QuoteVerifier 만으로는 TDX V4 quote를 verify 못 함. `MockAutomataDcap` 을 별도 deploy 하고 SGX/TDX hook 의 `setAutomataValidationAddr(mock)` 로 전환하여 DCAP cert chain 검증을 우회 (Option D 자체 검증과 직교).
2. **VSS dealing 무한 retry 루프** (Test 1 중간 발견): kernel 이 `vss: verifier already received a deal` 로 idempotent reject 했으나, story 측 `retry()` 헬퍼가 원본 error 를 `"all retries failed"` 로 덮어 substring 매칭 skip 패치가 미적용. `errors.Wrap(lastErr, "all retries failed")` 로 원본 보존 + dealing 패치에 `strings.Contains(err.Error(), "verifier already received a deal")` 로 idempotent skip 추가.

---

*문서 작성: 2026-05-10. 부록 C 추가: 2026-05-12. 추가 자료 또는 정정 사항이 있으면 PR로 업데이트 부탁드립니다.*
