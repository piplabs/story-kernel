...
is bound directly into `V4.report_data[0:32]` with `[32:64]` padded zero. No
Path-B bundle envelope is required on the kernel side; the on-chain
`TDXValidationHook` reads `V4.report_data[0:32]` directly.
...
too old, the TDX guest module is not loaded, or the host uses a paravisor
(in which case follow [paravisor.md](paravisor.md) instead).
...
If you see `tdx: using vendor adapter "paravisor"` instead, the host is
paravisor-mediated and the [paravisor.md](paravisor.md) flow applies. If
you see `tdx: no vendor adapter probe succeeded`, configfs-tsm is missing
— re-check Step 2.
...
Code commitment for direct-mode TDX is `keccak256(MRTD || RTMR0 || RTMR1 ||
RTMR2 || RTMR3)`. Read it from the kernel's first quote (printed in logs)
or compute it from the TDX module measurement on the host. Submit it to the
DKG contract via `whitelistEnclaveType(bytes32(2), {codeCommitment,
TDXValidationHookProxy}, true)` (operator action, see story contracts
repo).
