...
func buildSealPolicy(tpm io.ReadWriter, providers []ProviderPolicy) ([]byte, *sealPolicyMeta, error) {
	...
	// The PCR seal policy is fixed to a single bank (pcrPolicyHash); reject any
	// provider that strays from it rather than silently using providers[0].
	hashAlg := pcrPolicyHash
	for _, p := range providers {
		if p.Hash != hashAlg {
			return nil, nil, fmt.Errorf("tdx: provider %q Hash %v != pcrPolicyHash %v", p.Name, p.Hash, hashAlg)
		}
	}
...
func tdxSeal(tpm io.ReadWriter, plaintext []byte, providers []ProviderPolicy) ([]byte, error) {
	...
	// 1) Acquire AES key + nonce from the TPM RNG, then XOR in OS CSPRNG
	//    entropy (mixOSRandom). Routing through the TPM RNG keeps the trust
	//    chain inside the same root that holds the sealed object; mixing
	//    crypto/rand means a weak or backdoored in-TD vTPM RNG alone cannot
	//    determine the key/nonce (the result is as strong as the stronger
	//    source). A fresh key is drawn per seal, so each key encrypts exactly
	//    one message — nonce reuse across seals is harmless.
	//
	...
	if err := mixOSRandom(keyBytes); err != nil {
		return nil, fmt.Errorf("mix OS random into key: %w", err)
	}
	...
	if err := mixOSRandom(nonceBytes); err != nil {
		return nil, fmt.Errorf("mix OS random into nonce: %w", err)
	}
	...
	// 3) Build header for AAD binding before AES-GCM seal. bootstrap_flag and
	//    provider_count are AAD-bound (tamper-evident) but ADVISORY at unseal:
	//    unseal dispatches bootstrap-vs-strict from the compiled supportedProviders
	//    (len(populated)), not from these header bytes. provider_count is purely
	//    diagnostic; bootstrap_flag is cross-checked only for the strict-blob-on-
	//    bootstrap-host guard in tdxUnseal.
...
func tdxUnseal(tpm io.ReadWriter, blob []byte, providers []ProviderPolicy) ([]byte, error) {
	if len(providers) == 0 {
		return nil, errors.New("tdx: no providers configured")
	}
	...
	if len(populated) == 0 {
		// Diagnostic guard: a blob sealed in STRICT mode (bootstrap flag
		// clear) but unsealed on a host whose providers carry no
		// ExpectedDigest would silently take the bootstrap path. The TPM
		// still fails closed if PCRs don't match, but surface a clear
		// mismatch instead of a confusing "bootstrap unseal" error. (The
		// flag byte is AAD-bound, so it cannot be tampered post-seal.)
		if header[hdrOffBootstrap] == 0x00 {
			return nil, errors.New("tdx: blob was sealed in strict mode but no provider has an ExpectedDigest " +
				"(host is in bootstrap mode) — configure supportedProviders ExpectedDigest to unseal")
		}
...
// mixOSRandom XORs len(dst) bytes from the OS CSPRNG (crypto/rand) into dst in
// place. The AES key and GCM nonce come from the in-TD vTPM RNG; mixing the OS
// CSPRNG makes the result at least as strong as the stronger of the two
// sources, so a weak or backdoored vTPM RNG alone cannot determine them.
func mixOSRandom(dst []byte) error {
	osr := make([]byte, len(dst))
