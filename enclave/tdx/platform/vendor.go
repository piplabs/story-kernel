...
// environment (configfs-tsm direct, paravisor-mediated guest, GCP,
// IBM, ...).
//
...
type Vendor interface {
	...
	// STORY_TDX_VENDOR override value (e.g., "direct", "paravisor").
...
// first vendor whose Probe() succeeds wins, so register
// upstream-blessed paths (direct/configfs-tsm) before paravisor-
// specific adapters.
func Order() []Vendor {
	registryMu.Lock()
	defer registryMu.Unlock()
...
// SnapshotForTesting captures the current registry state as opaque
// values that RestoreForTesting can re-apply. Intended for unit tests
// that mutate the registry and need clean-up. Production code MUST
// NOT call either function — the names are deliberately verbose so
// reviewers spot misuse during code review.
//
// Snapshot/restore is preferred over a Reset hook because tests in the
// parent package (enclave/tdx) need to register fakes without losing
// the production "direct" vendor registered at process init. Restoring
// the snapshot brings the registry back to the post-init baseline.
...
// returned by SnapshotForTesting. See that function's doc for caveats.
