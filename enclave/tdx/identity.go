...
// cloneIdentity returns a deep copy of an enclave.Identity so callers cannot
// mutate the cached self-identity through the returned pointer.
func cloneIdentity(id *enclave.Identity) *enclave.Identity {
	if id == nil {
		return nil
	}
	return &enclave.Identity{
		Type:           id.Type,
		CodeCommitment: cloneSlice(id.CodeCommitment),
		ProductID:      cloneSlice(id.ProductID),
		MRTD:           cloneSlice(id.MRTD),
		RTMR0:          cloneSlice(id.RTMR0),
		RTMR1:          cloneSlice(id.RTMR1),
		RTMR2:          cloneSlice(id.RTMR2),
		RTMR3:          cloneSlice(id.RTMR3),
	}
}
