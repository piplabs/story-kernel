...
func TestBuildSealPolicy_HeterogeneousHash(t *testing.T) {
	t.Parallel()
	tpm := newTestTPM(t)
	providers := []ProviderPolicy{
		{Name: "a", PCRSelection: tpm2.PCRSelection{Hash: tpm2.AlgSHA256, PCRs: []int{7}}, ExpectedDigest: bytes.Repeat([]byte{1}, 32)},
	...
	require.ErrorContains(t, err, "heterogeneous Hash")
}
