...
func TestBackend_GetSelfIdentity_TDXShape(t *testing.T) {
	...
	require.Len(t, id.CodeCommitment, 5*sizeMeasurement)

	// CodeCommitment must equal MRTD || RTMR0..3 (native concatenation).
	want := append([]byte(nil), id.MRTD...)
	want = append(want, id.RTMR0...)
	want = append(want, id.RTMR1...)
	want = append(want, id.RTMR2...)
	want = append(want, id.RTMR3...)
	require.True(t, bytes.Equal(want, id.CodeCommitment))
}

func TestBackend_GetSelfCodeCommitment_240B(t *testing.T) {
	t.Parallel()
	b, _ := newTestBackend(t)
	c, err := b.GetSelfCodeCommitment()
	require.NoError(t, err)
	require.Len(t, c, 240, "TDX code commitment is MRTD||RTMR0..3 = 5 × 48B = 240B")
}
...
func TestBackend_SelfCheckFailure_PropagatesPerCall(t *testing.T) {
	...
	err5 := b.ValidateCodeCommitment(make([]byte, 240))
...
// TestTDX_GetSelfEnclaveInfo_PropagatesError verifies that
// enclave.GetSelfEnclaveInfo returns a non-nil error when the TDX backend
// is running with stub providers.
func TestTDX_GetSelfEnclaveInfo_PropagatesError(t *testing.T) {
	t.Parallel()
...
func TestTDX_ValidateCodeCommitment_PropagatesError(t *testing.T) {
	...
	// 240 bytes mirrors the native TDX code commitment shape
	// (MRTD || RTMR0..3); the comparison fails before length matters
	// because the local backend cannot read its own identity.
	err := enclave.ValidateCodeCommitment(make([]byte, 240))
	require.Error(t, err)
}
