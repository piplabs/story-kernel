...
// newTestTPM returns a fresh TPM2 simulator with PCRs 7, 11, and 12 extended
// to non-zero values. The startup self-check rejects all-zero PCRs, so tests
// that go through self-check must extend at least the configured PCRs. PCR 12
// stands in for the initrd-driven ELF measurement in production.
func newTestTPM(t *testing.T) TPMDevice {
	...
	extendPCR(t, sim, 12, []byte("test-pcr-12-binary-stub"))
	return sim
}
