...
// GetSelfIdentity returns the SGX-shaped Identity for the running enclave.
//
// The returned Identity contains defensive copies of the cached UniqueID and
// ProductID so caller-side mutation cannot corrupt the process-wide
// selfEnclaveInfo cache. This mirrors the TDX backend's cloneSlice/cloneIdentity
// pattern and matches the behavioral contract of GetSelfCodeCommitment.
func (Backend) GetSelfIdentity() (*enclave.Identity, error) {
	...
	return &enclave.Identity{
		Type:           enclave.IdentitySGX,
		CodeCommitment: bytes.Clone(info.UniqueID),
		ProductID:      bytes.Clone(info.ProductID),
	}, nil
}
