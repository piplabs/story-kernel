...
func TestDKGHappyPath_3Nodes(t *testing.T) {
	...
	require.False(t, bytes.Equal(shares[0], shares[2]), "node 0 and 2 shares should differ")

	// Verify finalization signatures
	var codeCommitment [32]byte
	copy(codeCommitment[:], cluster.CodeCommitment)

	regs, err := cluster.MockQC.GetAllParticipantDKGRegistrations(context.Background(), hex.EncodeToString(cluster.CodeCommitment), cluster.Round)
