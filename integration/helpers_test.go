...
func (c *DKGTestCluster) ProcessAllDeals() {
	...
	for _, dealResp := range c.DealResponses {
		for _, deal := range dealResp.GetDeals() {
			...
			require.Less(c.t, idx, n, "deal RecipientIndex %d out of range for %d nodes", idx, n)
...
// verifyPartialDecryptSignature verifies the ECDSA signature on a PartialDecryptTDH2Response.
// It reconstructs the signed message from the response fields, recovers the signer's public key,
// and checks it matches the expected CommPubKey from registration.
//
// Signed message layout (must match service/dkg_partial_decrypt.go signPartialDecryptResponse):
//   keccak256(round(4B) || ciphertext || encryptedPartial || ephPubKey || pubShareBz)
func verifyPartialDecryptSignature(
	t *testing.T,
	resp *pb.PartialDecryptTDH2Response,
	round uint32,
	ciphertext []byte,
	expectedCommPubKey []byte, // 65-byte uncompressed secp256k1
) bool {
	t.Helper()

	// Reconstruct the signed data (must match signPartialDecryptResponse encoding)
	encoded := make([]byte, 0, 4+len(ciphertext)+len(resp.GetEncryptedPartialDecryption())+len(resp.GetEphemeralPubKey())+len(resp.GetPubShare()))
	encoded = append(encoded, uint32ToBytes(round)...)
	encoded = append(encoded, ciphertext...)
	encoded = append(encoded, resp.GetEncryptedPartialDecryption()...)
	encoded = append(encoded, resp.GetEphemeralPubKey()...)
	encoded = append(encoded, resp.GetPubShare()...)
	respHash := ecrypto.Keccak256(encoded)

	sig := make([]byte, len(resp.GetSignature()))
	copy(sig, resp.GetSignature())
	// Normalize Ethereum v-value (27/28 -> 0/1) for SigToPub
	if sig[64] >= 27 {
		sig[64] -= 27
	}

	pubKey, err := ecrypto.SigToPub(respHash, sig)
	if err != nil {
		t.Logf("SigToPub failed: %v", err)
		return false
	}

	recoveredAddr := ecrypto.PubkeyToAddress(*pubKey)
	commAddr := ecrypto.PubkeyToAddress(*mustPubKeyFromBytes(expectedCommPubKey))
	return bytes.Equal(recoveredAddr.Bytes(), commAddr.Bytes())
}
...
// buildDealsByRecipient groups deals from all GenerateDealsResponse results into
// per-recipient slices indexed by RecipientIndex. This is a common pre-processing
// step before calling ProcessDeals on each node.
func buildDealsByRecipient(t *testing.T, n int, dealResponses []*pb.GenerateDealsResponse) [][]*pb.Deal {
	t.Helper()
	byRecipient := make([][]*pb.Deal, n)
	for _, dr := range dealResponses {
		for _, deal := range dr.GetDeals() {
			idx := int(deal.GetRecipientIndex())
			require.Less(t, idx, n, "deal RecipientIndex %d out of range for %d nodes", idx, n)
