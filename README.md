...
- **Remote Attestation**: Generates and verifies hardware-signed quotes (DCAP for SGX, V4 quote / Path-B bundle for TDX) for trust establishment
...
- **[enclave/tdx/README.md](enclave/tdx/README.md)** — Intel TDX overview (runtime requirements,
  vTPM-in-TCB requirement, the `supportedProviders` PolicyOR flow, bootstrap mode), with
  per–guest-interface setup guides:
  - **[enclave/tdx/setup/direct.md](enclave/tdx/setup/direct.md)** — direct configfs-tsm path
    (bare-metal, GCP CVMs, IBM Cloud, OCI, future AWS TDX).
  - **[enclave/tdx/setup/paravisor.md](enclave/tdx/setup/paravisor.md)** — paravisor-mediated path
    (currently OpenHCL on Hyper-V; used by Azure CVM TDX). Includes the empirically observed
    pitfalls from devnet bring-up.
...
│   └── tdx/          # TDX backend (configfs-tsm direct + paravisor adapters, vTPM PolicyOR seal)
